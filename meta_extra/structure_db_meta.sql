...
CREATE TABLE IF NOT EXISTS `k-tn` (
  ...
  KEY `vr` (`vr`),
  KEY `ses` (`ses`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Наборы на кириллице' AUTO_INCREMENT=0 ;
...
CREATE TABLE IF NOT EXISTS `k-ts` (
  ...
  KEY `f` (`f`),
  KEY `kol` (`kol`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Слова на кириллице' AUTO_INCREMENT=0 ;
...
CREATE TABLE IF NOT EXISTS `l-ts` (
  ...
  UNIQUE KEY `s` (`s`),
  KEY `f` (`f`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Слова на латинице' AUTO_INCREMENT=0 ;
...
CREATE TABLE IF NOT EXISTS `k-t_s` (
  ...
  KEY `id_s` (`id_s`),
  KEY `id_n` (`id_n`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Связи для кириллицы' AUTO_INCREMENT=0 ;
