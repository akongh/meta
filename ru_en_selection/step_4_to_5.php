...
if (isset($_POST["arr_kws_marked"])) {
    ...
    $_SESSION["arr_kws_assembled"] = $_SESSION["arr_kws_ordered"];
    ...
    foreach ($_SESSION["arr_kws_ordered"] as $kw) {
        ...
        if (!$mysqli_stmt_translation->bind_param("s", $kw)) {
            echo PHP_EOL . $mysqli_stmt_translation->errno . PHP_EOL . $mysqli_stmt_translation->error . PHP_EOL;
        }
        ...
        if (!$mysqli_stmt_statuses->bind_param("s", $kw)) {
            echo PHP_EOL . $mysqli_stmt_statuses->errno . PHP_EOL . $mysqli_stmt_statuses->error . PHP_EOL;
        }
...
header("Location: //" . $_SERVER["HTTP_HOST"] . "/ru_en_selection/step_5.php");
