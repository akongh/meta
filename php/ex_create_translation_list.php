...
$arr_kws_ru = $_POST["arr_kws_marked"];
...
for ($i = 0; $i < count($arr_kws_ru); $i++) {
    if (!$mysqli_stmt_translation->bind_param("s", $arr_kws_ru[$i])) {
    ...
    if (!$mysqli_stmt_statuses->bind_param("s", $arr_kws_ru[$i])) {
