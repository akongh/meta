...
--
-- Хост: localhost:3306
-- Время создания: Окт 24 2014 г., 12:23
...
CREATE TABLE IF NOT EXISTS `k-tn` (
  ...
  KEY `vr` (`vr`),
  KEY `ses` (`ses`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Наборы на кириллице' AUTO_INCREMENT=314335 ;
...
CREATE TABLE IF NOT EXISTS `k-ts` (
  ...
  UNIQUE KEY `s` (`s`),
  KEY `ss` (`s`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Слова на кириллице' AUTO_INCREMENT=4970694 ;
...
CREATE TABLE IF NOT EXISTS `k-t_s` (
  ...
  KEY `id_s` (`id_s`),
  KEY `id_n` (`id_n`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Связи для кириллицы' AUTO_INCREMENT=4964545 ;
...
CREATE TABLE IF NOT EXISTS `l-tn` (
  ...
  KEY `vr` (`vr`),
  KEY `ses` (`ses`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Наборы на латинице' AUTO_INCREMENT=4444 ;
...
CREATE TABLE IF NOT EXISTS `l-ts` (
  ...
  UNIQUE KEY `s` (`s`),
  KEY `ss` (`s`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Слова на латинице' AUTO_INCREMENT=48335 ;
...
CREATE TABLE IF NOT EXISTS `l-t_s` (
  ...
  KEY `id_s` (`id_s`),
  KEY `id_n` (`id_n`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Связи для латиницы' AUTO_INCREMENT=47287 ;
...
CREATE TABLE IF NOT EXISTS `tyrki` (
  ...
  `f` tinyint(3) unsigned zerofill NOT NULL
...
CREATE TABLE IF NOT EXISTS `tz` (
  ...
  `z` varchar(200) NOT NULL COMMENT 'Значение слова',
  PRIMARY KEY (`idz`),
  UNIQUE KEY `z` (`z`),
  KEY `zz` (`z`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=350 ;
